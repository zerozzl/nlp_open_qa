...
def retrieve(es, es_index, es_doc_type, page_size, match_name, match_value):
...
def main(args):
    ...
    with codecs.open('%s/log.txt' % output_path, 'w', 'utf-8') as fout:
        ...
        for split in data_splits:
            total_num = 0
            success_num = 0
            with codecs.open('%s/%s/%s/%s_seg.txt' % (args.data_path, dataset_path[0], dataset_path[1], split),
                             'r', 'utf-8') as fin:
                for line in tqdm(fin):
                    ...
                    success = False
                    ret_segments = retrieve(es, es_index, args.es_doc_type, args.retrieve_size, args.token_type, text)
                    for ret_seg in ret_segments:
                        ret_seg = ret_seg['_source']
                        if ret_seg['id'] in segments_id:
                            success = True
                            break
